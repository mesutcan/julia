...
void *gc_mark_label_addrs[gc_mark_nlabels];
                                          ...
                                          jl_value_t *obj)
{
    ...
    gc_mark_stack_push(gc_cache, sp, gc_mark_label_addrs[1], &data, sizeof(data), 1);
}
...
STATIC_INLINE int gc_mark_queue_obj(jl_gc_mark_cache_t *gc_cache, gc_mark_sp_t *sp, void *_obj)
{
    ...
    if (!gc_try_setmark(obj, &nptr, &tag, &bits))
        return (int)nptr;
    gc_mark_marked_obj_t data = {obj, tag, bits};
    gc_mark_stack_push(gc_cache, sp, gc_mark_label_addrs[0], &data, sizeof(data), 1);
    return (int)nptr;
}
...
NOINLINE void *const *gc_mark_loop(jl_ptls_t ptls, gc_mark_sp_t sp)
{
    static void *const label_addrs[] = {
        gc_mark_laddr(marked_obj), gc_mark_laddr(scan_only), gc_mark_laddr(finlist),
        gc_mark_laddr(objarray), gc_mark_laddr(obj8), gc_mark_laddr(obj16),
        gc_mark_laddr(obj32), gc_mark_laddr(stack), gc_mark_laddr(module_binding)
    };
...
mark: {
        ...
        if (vt == jl_simplevector_type) {
            ...
            assert(nfields > 0);
            jl_fielddesc8_t *desc_end = desc + nfields;
            gc_mark_obj8_t markdata = {new_obj, desc, desc_end, (9 << 2) | 1 | bits};
            ...
            if (s) {
                ...
                gc_mark_stack_push(&ptls->gc_cache, &sp, gc_mark_laddr(stack),
                                   &stackdata, sizeof(stackdata), 0);
                sp.data += sizeof(stackdata);
                goto stack;
            }
            else {
